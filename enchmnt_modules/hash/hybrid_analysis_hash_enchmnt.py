    ...
    try:
        ...
        if json_response:
            if "validation_errors" in json_response:
                add_data(session, hash_to_look, "hybridanalysis", "IOC not found.", "hash_table")
                logger.info("HybridAnalysis info failed.")
            else:
                add_data(session, hash_to_look, "hybridanalysis", json_response, "hash_table")
                logger.info("HybridAnalysis info added.")
    except Exception as e:
        add_data(session, hash_to_look, "hybridanalysis", "Error occurred.", "hash_table")
